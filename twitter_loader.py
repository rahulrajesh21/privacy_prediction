...
class TwitterDataLoader:
    def __init__(self, data_dir='twitter', sample_size=5):
        """
        Initialize the Twitter data loader.
        
        Args:
            data_dir: Directory containing Twitter data
            sample_size: Number of ego networks to sample (to avoid using the whole dataset)
        """
        self.data_dir = data_dir
        self.ego_ids = self._get_ego_ids()
        
        # Sample a subset of ego networks if specified
        if sample_size and sample_size < len(self.ego_ids):
            np.random.seed(42)  # For reproducibility
            self.ego_ids = np.random.choice(self.ego_ids, sample_size, replace=False).tolist()
            print(f"Sampled {sample_size} Twitter ego networks: {self.ego_ids}")
    ...
    def load_ego_network(self, ego_id):
        ...
        with open(circles_file, 'r') as f:
            for line in f:
                ...
                circle_name = parts[0]
                members = [int(x) for x in parts[1:]] if len(parts) > 1 else []
        ...
        data = Data(
            ...
            ego_id=int(ego_id),
            platform="twitter"  # Add platform identifier
        )
    ...
    def load_all_networks(self):
        ...
        all_networks = []
        for ego_id in tqdm(self.ego_ids, desc="Loading Twitter ego networks"):
            try:
                network = self.load_ego_network(ego_id)
                all_networks.append(network)
            except Exception as e:
                print(f"Error loading Twitter ego network {ego_id}: {e}")
    ...
    def create_privacy_labels(self, data_list, sensitive_attributes):
        ...
        for data in data_list:
            ...
            # Twitter-specific: Add higher weight to exposure score for Twitter
            # as Twitter is more public than Facebook
            privacy_risk = 0.8 * exposure_score + 0.2 * norm_degrees
